...
def table_header(df, file_excel):
    ...
    try:
        df = df.loc[:,['Уровень', sign_1c, 'Вид связи КА за период', 'Дебет_начало', 'Кредит_начало', 'Дебет_оборот', 'Кредит_оборот', 'Дебет_конец', 'Кредит_конец']]
    except KeyError:
        df = df.loc[:,['Уровень', sign_1c, 'Дебет_начало', 'Кредит_начало', 'Дебет_оборот', 'Кредит_оборот', 'Дебет_конец', 'Кредит_конец']]

    if df[df[sign_1c] == 'Итого'][['Дебет_начало', 'Кредит_начало']].empty:
        print('Нет строки ИТОГО в таблице, чтобы свериться!!!!!')
        return sign_1c, None
    else:
        print('будем сверяться!!!!!')
        return sign_1c, df[df[sign_1c] == 'Итого'][['Дебет_начало', 'Кредит_начало']].copy()
